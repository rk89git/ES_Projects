...
public class RealTimeListingPageConsumerRunnableTask implements Runnable {

	private DBConfig config = DBConfig.getInstance();

	private static Logger log = LogManager.getLogger(RealTimeListingPageConsumerRunnableTask.class);
	...
	public RealTimeListingPageConsumerRunnableTask(int threadIndex, List<String> topics, Properties props) {
		this.threadIndex = threadIndex;
		this.topics = topics;
		props.setProperty(ConsumerConfig.GROUP_ID_CONFIG,config.getString("kafka.groups.id")+ "Listing");
	...
	private void processMessage(byte[] message) {
		try {
			...
			String url_foldername = (String) msg.get(Constants.URL_FOLDERNAME);
			String splTracker="-1";
...
*/
			...
			String storyRowId = storyId + "_"+url_foldername+"_" + msg.get(Constants.TRACKER) + "_"+splTracker+"_" + msg.get(Constants.REF_PLATFORM) + "_"
					+ msg.get(Constants.VERSION) + "_" + msg.get(Constants.HOST) + "_" + date;
			...
			//if (StringUtils.isNotBlank(storyId) && Integer.valueOf(storyId) != 0) {
				...
				upvsMap.put(Constants.ROWID, session_id + "_" + storyId+"_"+url_foldername+"_" +sess_id+ "_"+splTracker+"_"+DateUtil.getCurrentDate().replaceAll("_", "-"));
...
*/
	...
	private void insertData(String indexDate) {
		...
		String upvsTableName = "realtime_upvs_listing_" + indexDate;
		resetTime();
		...
		*/// Index the summary data of unique story in unique_userstory
		 ...
		 * tableName, MappingTypes.MAPPING_REALTIME_UNIQUE_USER_STORY,
		 * summaryMessages);
		 */
		...
		*/
		...
		log.info("Thread " + threadIndex + ", Records inserted in realtime upvs listing mapping index, size: "
				+ upvsData.size());
		log.info(upvsTableName + "-" + MappingTypes.MAPPING_REALTIME + " record count "
				+ upvsData.size() + "; Size of storyIds for PV Update " + storyIdsforCounterUpdate.size());
		upvsData.clear();


		// Index the story details in story_detail index
		if (storyDetailsData.size() > 0) {
			elasticSearchIndexService.indexOrUpdate(IndexUtils.getYearlyIndex(Indexes.STORY_DETAIL_LISTING), MappingTypes.MAPPING_REALTIME,
					storyDetailsData);
			log.info("Thread " + threadIndex + ", Records inserted in story_detail_listing index, size: "
...
*/
		if (isPrd) {
			...
			if (storyDetailRowIDMapForUVCounterUpdate.size() > 0) {
				elasticSearchIndexService.incrementCounter(IndexUtils.getYearlyIndex(Indexes.STORY_DETAIL_LISTING), MappingTypes.MAPPING_REALTIME,
						storyDetailRowIDMapForUVCounterUpdate, Constants.UVS);
				log.info("Thread " + threadIndex + ", UVs updated in story_detail_listing index, size: "
		...
		if (storyIdsforCounterUpdate.size() > 0) {
			elasticSearchIndexService.incrementCounter(IndexUtils.getYearlyIndex(Indexes.STORY_DETAIL_LISTING), MappingTypes.MAPPING_REALTIME,
					storyDetailRowIdMapForPvCounterUpdate, Constants.PVS);
			log.info("Thread " + threadIndex + ", PVs updated in story_detail_listing index, size: "
