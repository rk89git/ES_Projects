...
public class ReProcess {
	...
	private static Logger log = Logger.getLogger(ReProcess.class);

	int batchSize = 5000;
	//static String ip = "104.199.156.183";
	static String ip="10.140.0.7";
	...
	public void reindex(String sourceIndex, String targetIndex) {
		...
		try {
			...
			SearchResponse scrollResp = client.prepareSearch(sourceIndex)
					...
					//.setQuery(QueryBuilders.matchQuery("_id", "294260637288_10155737468972289_521_2017-05-13_13"))
				...
				for (SearchHit hit : scrollResp.getHits().getHits()) {
					try {
						...
						if (hit.getSource().get("cat_id") != null) {
							if (hit.getSource().get("cat_name") == null
									|| hit.getSource().get("cat_name").toString().isEmpty()) {

								//	System.out.println(hit.getId()+
								//		" cat name was not there for cat id" + hit.getSource().get("cat_id").toString());
	...
	public void reindexDbVideos(String sourceIndex, String targetIndex) {
		...
		try {
			...
			SearchResponse scrollResp = client.prepareSearch(sourceIndex)
					//.setSearchType(SearchType.SCAN)
					.addSort("_doc",SortOrder.ASC)
					.setQuery(QueryBuilders.termQuery(Constants.STATUS_TYPE, "video"))
					.setScroll(new TimeValue(60000)).setSize(numRec).execute().actionGet();
			List<Map<String, Object>> listWeightage = new ArrayList<Map<String, Object>>();
			AtomicInteger dayWiseThreadAtomicInt = new AtomicInteger(1);
			while (true) {
				...
				for (SearchHit hit : scrollResp.getHits().getHits()) {
					try {
						...
						record.put("ga_cat_name", record.get("DBVideo"));
						record.put(Constants.ROWID, hit.getId());
	...
	public void reindexDayHyper(String sourceIndex, String targetIndex) {
		long startTime = System.currentTimeMillis();
		try {
			int numRec = 5000;
	...
	public static void main(String[] args) {
		...
		// duvi.getMySSQLCOnnection("10.140.0.7", "3306", "web2", "web2",
		// "realtime");
		duvi.reindex(args[0], args[1]);
		//duvi.reindexDbVideos(args[0], args[1]);
		System.out.println("===============================Starting reindexDayHyper========================================");
		duvi.reindexDayHyper(args[0], args[1]);
	}
